...
- **Open an issue:** Go to the Issues tab and click **New issue**, then pick the relevant template (bug, feature/wish, or question).
- **Search first:** Avoid duplicates by checking existing issues.
- **Add detail:** Include Distro version, TM1/PA version, OS, steps to reproduce (for bugs), screenshots/logs, and impact level.
- **Follow up:** Respond to clarification questions so we can triage quickly.

See `CONTRIBUTING.md` and `SUPPORT.md` for more guidance.
...
- **Contributing:** Read `CONTRIBUTING.md` for issue/PR guidance.
- **Code of Conduct:** See `CODE_OF_CONDUCT.md` for community standards.
- **License:** This helpdesk repository is under the `MIT` license (see `LICENSE`).
